...
check-safety:
	...
	$(BANDIT_COMMAND_FLAG)poetry run bandit -ll -r thanakan/
...
check-style:
	...
	$(MYPY_COMMAND_FLAG)poetry run mypy --config-file setup.cfg thanakan tests/**/*.py

.PHONY: codestyle
codestyle:
	-poetry run pyupgrade --py39-plus **/*.py
