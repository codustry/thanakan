...
async def test_slip_vert(scb_api):
    out = await scb_api.verify_slip(
        transaction_ref_id="202110268HbTUKyFqZ5vKxiUx", sending_bank_id="014"
    )
    print(out)
...
async def test_create_deeplink(scb_api, deeplink_payload):
    out = await scb_api.create_deeplink(payload=deeplink_payload)
    print(out)
...
async def test_query_deeplink(scb_api, deeplink_payload):
    ...
    out = await scb_api.get_deeplink(transaction_ref_id=out.data.transaction_id)
    print(out)
